...
# Load JMMI data
df_base <- read.csv("./inputs/data_new.csv", stringsAsFactors = FALSE, header = T, na.strings=c(""," ","NA"))
...
ba_xz <- ba_x %>%
  ...
  distinct() %>%
  separate(item, c("item", "var"), sep = "__")
# Create a temporary table with the sum of each group
ba_xz0 <- ba_xz %>%
  select(item, n) %>%
  group_by(item) %>%
  summarise(t = sum(n))
# Send back sum, so can be used to calculate new global percentages
ba_xz <- ba_xz %>%
  left_join(ba_xz0, by = "item") %>%
  unite("item", item, var, sep = "__") %>%
  mutate(pc = n/t) %>%
  select(!t)
...
# Export the file
saveWorkbook(OUT, paste0("HQ_JMMI_2020NOV_analysed_", ll,"_",Sys.Date() , ".xlsx"), overwrite=TRUE)
